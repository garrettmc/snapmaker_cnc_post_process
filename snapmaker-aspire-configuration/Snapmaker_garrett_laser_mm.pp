...
POST_NAME = "Snapmaker Laser - Garrett (mm) (*.cnc)"
...
+ with the command 'S' (so put out a P before the
...
+  G90 is absolute positioning (default). 
+  Moves up to Z10. G21 = millimeters (default).
+
+  Dec-20-24 - garrett - Got rid of M3, I don't
+  want the laser on immediately. I added M2003/M2004, 
+  as Luban seemed to add (it says it's for cross
...
+  so everythign is half power. I'm currently only
+  doing engraving, no cutting, so I think that's
+  good. 
+---------------------------------------------------
